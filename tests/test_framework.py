...
def test_table_validations(table):
    ...
    assert results["null_count"] > 0, f"{table_name}: No nulls found (expected test data)"
    assert results["duplicate_count"] > 0, f"{table_name}: No duplicates found (expected test data)"